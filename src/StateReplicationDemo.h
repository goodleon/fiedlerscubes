...
public:
	...
	void Initialize()
	{
		for ( int i = 0; i < MaxPlayers; ++i )
		{
			...
			config.cellWidth = GridSize / config.cellSize;
			config.cellHeight = GridSize / config.cellSize;
	...
	void Render( float deltaTime, bool shadows )
	{
		...
		for ( int i = 0; i < MaxPlayers; ++i )
		{
			...
			math::Vector lookat = origin[i] - math::Vector(0,0,1);
			#ifdef WIDESCREEN
			math::Vector position = lookat + math::Vector(0,-12.5,6);
			#else
			math::Vector position = lookat + math::Vector(0,-11,5.5);
			#endif
		...
		// hack for GDC demo
		// don't render remote view for the first second
		frames++;
		if ( frames > 62 )
		{
