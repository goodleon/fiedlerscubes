...
namespace net
{
	...
	public:
		...
		{
			this->ip = ( uint32_t(a) << 24 ) | ( uint32_t(b) << 16 ) | ( uint32_t(c) << 8 ) | uint32_t(d);
			this->port = port;
		}
 		...
 		uint32_t GetIP() const
		{
 		...
 		uint8_t GetA() const
		{
			return uint8_t( ip >> 24 );
		}
	
 		uint8_t GetB() const
		{
			return uint8_t( ip >> 16 );
		}
	...
	public:
		...
		bool Open( unsigned short port, int options = NonBlocking )
		{			
			assert( !IsOpen() );
		...
		bool Send( const Address & destination, const void * data, int size )
		{
			...
			assert( destination.GetIP() != 0 );
			assert( destination.GetPort() != 0 );
		
			sockaddr_in address;
			address.sin_family = AF_INET;
			address.sin_addr.s_addr = htonl( destination.GetIP() );
		...
		bool Select()
		{
			// note: this doesn't work yet - need to fix
			timeval tv;
			tv.tv_sec = 1;
			tv.tv_usec = 0;
			fd_set fd;
			FD_ZERO( &fd );
			FD_SET( socket, &fd );
			return select( FD_SETSIZE, &fd, NULL, NULL, &tv ) > 0;
		}
	...
	public:
		...
		bool Start( int port )
		{
			...
			if ( !socket.Open( port, Socket::Broadcast | Socket::NonBlocking ) )
				return false;
			running = true;
			return true;
		}
	...
	public:
		...
		{
			if ( bitpacker.GetMode() == BitPacker::Write )
				return "write";
			else
				return "read";
		}
		...
		bool SerializeLong( uint64_t & value )
		{
			uint32_t lo = uint32_t(value);
			uint32_t hi = uint32_t(value>>32);
			if ( !SerializeInteger( lo ) )
				return false;
			if ( !SerializeInteger( hi ) )
				return false;
		}

		/*
		bool SerializeLong( uint64_t & value, uint64_t min = 0, uint64_t max = 0xFFFFFFFFFFFFFFFFL )
		{
			#if 0
			assert( min < max );
			if ( IsWriting() )
			...
			if ( IsReading() )
			{
				...
				if ( value < min || value > max )
				{
					printf( "serialize long out of range (read)\n" );
					assert( false );
					return false;
				}
			}
		...
		*/
		...
		bool SerializeCompressedFloat( float & value, float minimum, float maximum, float resolution )
		{
			...
			if ( IsWriting() )
			{
				...
				integerValue = (unsigned int) math::floor( normalizedValue * maxIntegerValue + 0.5f );
			}
		...
		bool SerializeBits( unsigned int & value, int bits )
		{
			...
			{
				printf( "error: not enough bits remaining to %s bits (have %d bits, need %d bits)\n", 
					GetTag(), bitpacker.BitsRemaining(), bits );
				assert( false );
				return false;
			}
			if ( journal.IsValid() )
		...
		bool SerializeBytes( uint8_t array[], int bytes )
		{
			assert( bytes > 0 );
			if ( bitpacker.BitsRemaining() < bytes * 8 )
			{
				printf( "error: not enough bits remaining to %s bytes (have %d bits, need %d bits)\n", 
					GetTag(), bitpacker.BitsRemaining(), bytes * 8 );
				assert( false );
				return false;
			}
	...
	#define serialize_uncompressed_byte( stream, value ) do { if ( !stream.SerializeByte( value ) ) { printf( "error: failed to serialize uncompressed byte (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_short( stream, value ) do { if ( !stream.SerializeShort( value ) ) { printf( "error: failed to serialize uncompressed short (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_int( stream, value ) do { if ( !stream.SerializeInteger( value ) ) { printf( "error: failed to serialize uncompressed int (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_long( stream, value ) do { if ( !stream.SerializeLong( value ) ) { printf( "error: failed to serialize uncompressed long (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_float( stream,value ) do { if ( !stream.SerializeFloat( value ) ) { printf( "error: failed to serialize uncompressed float (%s)\n", stream.GetTag() ); return false; } } while(0)

	/*
	#define serialize_uncompressed_vector2( stream, value ) do { if ( !stream.SerializeVector2( value ) ) { printf( "error: failed to serialize vector (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_vector3( stream, value ) do { if ( !stream.SerializeVector3( value ) ) { printf( "error: failed to serialize vector (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_vector4( stream, value ) do { if ( !stream.SerializeVector4( value ) ) { printf( "error: failed to serialize vector (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_quaternion( stream, value ) do { if ( !stream.SerializeQuaternion( value ) ) { printf( "error: failed to serialize quaternion (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_uncompressed_transform( stream, value ) do { if ( !stream.SerializeTransform( value ) ) { printf( "error: failed to serialize transform (%s)\n", stream.GetTag() ); return false; } } while(0)
	*/

	#define serialize_bits( stream, value, bits ) do { UINT32 tmp = value; if ( !stream.SerializeBits( tmp, bits ) ) { printf( "error: failed to serialize bits (%s)\n", stream.GetTag() ); return false; } value = tmp; } while(0)
	#define serialize_bytes( stream, array, bytes ) do { if ( !stream.SerializeBytes( array, bytes ) ) { printf( "error: failed to serialize bytes (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_bool( stream, value ) do { if ( !stream.SerializeBoolean( value ) ) { printf( "error: failed to serialize boolean (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_byte( stream, value, min_value, max_value ) do { if ( !stream.SerializeByte( value, min_value, max_value ) ) { printf( "error: failed to serialize byte (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_short( stream, value, min_value, max_value ) do { if ( !stream.SerializeShort( value, min_value, max_value ) ) { printf( "error: failed to serialize short (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_int( stream, value, min_value, max_value ) do { if ( !stream.SerializeInteger( value, min_value, max_value ) ) { printf( "error: failed to serialize int (%s)\n", stream.GetTag() ); return false; } } while(0)
//	#define serialize_long( stream, value, min_value, max_value ) do { if ( !stream.SerializeLong( value, min_value, max_value ) ) { printf( "error: failed to serialize long (%s)\n", stream.GetTag() ); return false; } } while(0)
	#define serialize_float( stream, value, min_value, max_value, resolution ) do { if ( !stream.SerializeCompressedFloat( value, min_value, max_value, resolution ) ) { printf( "error: failed to serialize float (%s)\n", stream.GetTag() ); return false; } } while(0)

	#define serialize_enum( stream, enum_value, enum_type, enum_count ) do { int value = (int) enum_value; if ( !stream.SerializeInteger( value, 0, enum_count - 1 ) ) { printf( "error: failed to serialize enum\n" ); return false; } enum_value = (enum_type) value; } while(0)

	#define serialize_checkpoint( stream ) do { if ( !stream.Checkpoint() ) { printf( "error: checkpoint failed (%s)\n", stream.GetTag() ); return false; } } while(0)

	#define serialize_magic( stream, magic ) do { uint32_t value = (uint32_t) magic; stream.SerializeInteger( value ); assert( value == magic ); } while(0)
}
